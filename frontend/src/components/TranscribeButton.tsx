...
export function TranscribeButton(props: Props): JSX.Element {
    const { isModelLoading, isModelLoaded, isTranscribing, onClick, ...buttonProps } = props;
    return (
        <button
            {...buttonProps}
            onClick={(event) => {
        ...
        >
            {isModelLoading ? (
                <Spinner text={"Модель жүктөлүүдө..."} />
            ) : isTranscribing ? (
                <Spinner text={"Транскрипцияланууда..."} />
            ) : isModelLoaded ? (
                "Аудиону транскрипцияла"
            ) : (
                "Модельди жүктөө"
            )}
        </button>
